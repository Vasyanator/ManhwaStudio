...
- **`ms-gifs`** (`crates/ms-gifs`) — GUI-free хранилище встроенных анимированных
  ...
  egui и без тяжёлого `image` — загрузку текстур делает потребитель. `decode(Hint)` отдаёт
  `Animation { width, height, frames }`, где `Frame::rgba` — всегда НЕ премультиплицированный
  RGBA ровно `w*h*4` (RGB-выход декодера расширяется), а `Hint::name()` — стабильный
  ключ кэша (`"<категория>.<параметр>"`), который нельзя менять: на него завязан кэш
  потребителя. **`decode` ДОРОГОЙ** — до ~106 МБ RGBA на одну анимацию (кадры держатся
  разжатыми) — и запрещён на GUI-потоке; панику `image-webp` (`read_frame` ассертит
  анимированность и длину буфера) крейт снимает предварительной валидацией и
