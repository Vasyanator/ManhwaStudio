...
- **`ms-gifs`** (`crates/ms-gifs`) — GUI-free хранилище встроенных анимированных
  WebP-подсказок + их декодер. Ассеты (`assets/*.webp`, ~1.2 МБ) встроены через
  `include_bytes!` и разбиты по модулям-категориям, названным по вкладкам UI
  (`ms_gifs::typing::{KERNING, CHAR_WIDTH, CHAR_HEIGHT, LINE_SPACING}`), поэтому правки
  `src/` не пересобирают байты. Зависит только от `image-webp` + `thiserror`: без egui и
  без тяжёлого `image` — загрузку текстур делает потребитель. `decode(Hint)` отдаёт
  ...
  ключ кэша. **`decode` ДОРОГОЙ** (десятки МБ, ~100+ кадров) и запрещён на GUI-потоке;
  панику `image-webp` (`read_frame` ассертит анимированность и длину буфера) крейт
  снимает предварительной валидацией и типизированным `GifError`.
