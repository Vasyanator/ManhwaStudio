...
- **`ProjectPaths`** — нормализованные пути к `src/`, `clean_layers/`, `cleaned/`, `text_detection/`, `characters.json`, `terms.json`, `notes_file`, `bubbles.json`, `settings.json`, `wiki/`, `fonts/`, `alt_vers/`, `text_images/`.
- **`Bubble`** — `id`, `img_idx`, `img_u`, `img_v`, `side` (Left/Right), `bubble_type` (Default/Aside/OnTop), `text`, `original_text`, `extra: Map<String,Value>`.
...
- **`src/input_manager_v2.rs`** (current): `InputManagerV2` — поддерживает `KeyboardShortcut` и `ModifierOnly` (Ctrl/Alt/Shift) бинды; overrides из `user_config.json → Hotkeys`; `save_hotkey_override` / `clear_hotkey_override`.
...
**OCR engines**: MangaOCR, EasyOCR, PaddleOCR (Python ONNX backend), Surya OCR (Python backend, task-based recognition without boxes / block mode / with internal detection).
...
**BackendHttpClient**: persistent keep-alive TCP, retry при disconnect.
...
- The backend is a separate `ai_backend.py` process on `127.0.0.1`; `8765` is the default port,
  and Settings process management switches to a free runtime port with `--port` if the default is
  occupied.
- **Endpoints**: `/health`, `/device`, `/device/set`, `/device/cuda_diagnostics`, `/inpaint/lama_v2`, `/inpaint/aot`, OCR endpoints, `/textdetector/ctd/detect`, `/textdetector/paddle/detect`.
