...
- **Запуск из исходников**: `run-dev.{Linux.sh,MacOS.command,Windows.bat}` в корне — тонкие лаунчеры к `tools/run-dev/`. Скрипт обновляет рабочую копию из git (включая копию, распакованную из ZIP без `.git`), ставит изолированный Rust в `installer_files/rust` и запускает `cargo run --bin manhwastudio_rs --release`. Контракт и все ветки алгоритма — `dev-docs/run_dev_plan.md`
