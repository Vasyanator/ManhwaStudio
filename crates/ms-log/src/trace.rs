/*
FILE OVERVIEW: crates/ms-log/src/trace.rs
Opt-in detailed execution tracing, gated behind the app's `--trace` CLI flag.
...
-----------------------
Both macros are `#[macro_export]` at the `ms_log` crate root; the app re-exports
them (`pub use ms_log::{trace_log, trace_scope};`) so call sites keep short paths:

    ms_log::trace_log!(ms_log::trace::cat::LAYER_MODEL, "add_node page={} uid={}", page, uid);
    // or, from the app after the re-export shim:
    ...
    let _s = ms_log::trace_scope!(ms_log::trace::cat::RENDER, "render_text w={} h={}", w, h);

Tip: `use ms_log::trace::cat;` then `ms_log::trace_log!(cat::TYPING, "...")`.
