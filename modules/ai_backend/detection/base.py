"""
File: modules/ai_backend/detection/base.py

Compatibility base utilities for the vendored `textdetector/` modules.

This mirrors the lightweight BaseModule API used by the migrated CTD code.

Notes:
The single inbound edge is `detection/textdetector/base.py`, which imports
`BaseModule`, `DEFAULT_DEVICE` and `DEVICE_SELECTOR` from here as `from ..base
import ...`. Nothing else in the backend imports this file, so it is easy to
mistake for dead code — it is not. Keep it in the same package as
`textdetector/` so that relative import keeps resolving.
"""
