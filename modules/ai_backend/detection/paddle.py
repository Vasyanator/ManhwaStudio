...
- Return polygon blocks and a glyph-shaped binary mask for the
  `textdetector.paddle` IPC method (`ipc/handlers/textdetector.py`).
- Read detector weights from `ManhwaStudio_AI_Models/ONNX/PaddleOCR`.

Notes:
Model-path and ONNX Runtime provider resolution are owned by the shared
`engines/paddle_onnx.py` helper, which the Paddle OCR service also uses.
"""
