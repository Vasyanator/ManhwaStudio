"""
FILE OVERVIEW: modules/ai_backend/engines/paddle_onnx.py
...
from ..runtime.model_manager import LoadedModelManager, ModelUsageLease
...
def resolve_models_root() -> Path:
    """Locate `ManhwaStudio_AI_Models/ONNX/PaddleOCR`, preferring the CWD tree.

    Candidates are probed in a fixed order - current directory, program root,
    then the parent of each - and the first existing one wins. When none
    exists the first candidate is returned so that callers report a missing
    model against the expected in-CWD location rather than failing here.
    """
    current = Path.cwd().resolve()
    module_root = program_root()
    candidates = [
