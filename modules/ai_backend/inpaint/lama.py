"""
File: modules/ai_backend/inpaint/lama.py
...
- The `InpainterV2` runtime is loaded by FILE PATH (`spec_from_file_location`),
  not by import. Both the module file and its `saicinpainting` bundle are
  siblings of THIS file, so their locations are derived from `__file__` here;
  see `_resolve_inpainter_source_path` / `_prepare_runtime_paths`.
"""
...
try:
    ...
    LAMA_DIR = str(program_root() / "ManhwaStudio_AI_Models" / "Torch" / "LaMa")
...
#   `modules/ai_backend/inpaint/lama_v2_runtime_inpainter.py` через путь к файлу
...
class LamaInpaintService:
    ...
    @staticmethod
    def _resolve_inpainter_source_path() -> Path:
        """Return the path of the standalone `InpainterV2` runtime module.

        The module is a SIBLING of this file and is loaded by path through
        `importlib.util.spec_from_file_location`, so the location is derived
        from `__file__` and never from the program root: a package move then
        carries the pair along instead of silently breaking the lazy load. A
        wrong path produces no import error at all - only a `FileNotFoundError`
        the first time a user actually runs LaMa V2 inpainting.

        # Raises
        `FileNotFoundError` when the runtime module is missing from the package.
        """
        source = Path(__file__).resolve().parent / "lama_v2_runtime_inpainter.py"
        if source.is_file():
            return source
    ...
    @staticmethod
    def _prepare_runtime_paths(module_path: Path) -> None:
        """Put the LaMa V2 runtime dependencies on `sys.path` before the load.

        `module_path` is the runtime module returned by
        `_resolve_inpainter_source_path`; it is executed outside the package, so
        its plain `import saicinpainting` must be satisfied from `sys.path`.
        """
        # `lama_runtime_bundle/` (the vendored `saicinpainting`) is a sibling of
        # this file, exactly like the runtime module itself - deriving both from
        # `__file__` is what keeps them from drifting apart on a package move.
        # The program root is appended separately so the runtime module can also
        # `import config`.
        package_dir = Path(__file__).resolve().parent
        runtime_paths = [
            package_dir / "lama_runtime_bundle",
            module_path.parent,
            Path(str(_PROGRAM_DIR)),
        ]
