"""
FILE HEADER (saicinpainting/training/trainers/base.py)
- Назначение: базовый Lightning-модуль LaMa для train/infer сценариев.
- Для backend-инференса в MangaFucker важен режим `predict_only=True`:
  - должен собираться только `generator`;
  - лишние train/eval зависимости импортируются лениво (только если реально нужен train).
- Это позволяет держать runtime_bundle компактнее и не тянуть неиспользуемые
  для инференса модули при обычном `load_checkpoint(..., predict_only=True)`.
"""
...
class BaseInpaintingTrainingModule(ptl.LightningModule):
    def __init__(self, config, use_ddp, *args,  predict_only=False, visualize_each_iters=100,
        ...
        # параметры клиппинга из конфига (куда вы их поместили)
        self._clip_val = getattr(getattr(config, "model", config), "gradient_clip_val", 0.0)
        self._clip_alg = getattr(getattr(config, "model", config), "gradient_clip_algorithm", "norm")
        ...
        if not predict_only:
            ...
            from saicinpainting.training.modules import make_discriminator
    ...
    def training_step(self, batch, batch_idx):
        """
        Manual optimization для двух оптимизаторов: сначала G, затем D (если используется).
        """
        self._is_training_step = True
        opt_g, opt_d = self.optimizers()  # порядок соответствует configure_optimizers

        # === Шаг генератора ===
        set_requires_grad(self.generator, True)
        set_requires_grad(self.discriminator, False)
        out_g = self._do_step(batch, batch_idx, mode='train', optimizer_idx=0)

        loss_g = out_g['loss']
        # Lightning manual opt:
        self.manual_backward(loss_g)
        # ручное клиппирование для G
        if self._clip_val and self._clip_val > 0:
            self.clip_gradients(
                opt_g, gradient_clip_val=float(self._clip_val), gradient_clip_algorithm=str(self._clip_alg)
            )
        opt_g.step()
        
        opt_g.zero_grad()

        # апдейт EMA/average генератора (то, что раньше было в training_step_end)
        if self.training and self.average_generator \
                and self.global_step >= self.average_generator_start_step \
                and self.global_step >= getattr(self, "last_generator_averaging_step", -1) + self.average_generator_period:
        ...
        # === Шаг дискриминатора (если нужен) ===
        out_d = {}
        if self.config.losses.adversarial.weight > 0:
            set_requires_grad(self.generator, False)
            set_requires_grad(self.discriminator, True)
            out_d = self._do_step(batch, batch_idx, mode='train', optimizer_idx=1)
            loss_d = out_d['loss']
            self.manual_backward(loss_d)
            # ручное клиппирование для D
            if self._clip_val and self._clip_val > 0:
                self.clip_gradients(
                    opt_d, gradient_clip_val=float(self._clip_val), gradient_clip_algorithm=str(self._clip_alg)
                )
            opt_d.step()
            opt_d.zero_grad()
        self.log("train_g_loss", loss_g.detach(), on_step=True, prog_bar=True, logger=True, sync_dist=True)
        if self.config.losses.adversarial.weight > 0:
            self.log("train_d_loss", loss_d.detach(), on_step=True, prog_bar=False, sync_dist=True)

        # логируем learning rate (важно!)
        opt_g, opt_d = self.optimizers()
        self.log("lr_g", opt_g.param_groups[0]["lr"], on_step=True, prog_bar=False)
        self.log("lr_d", opt_d.param_groups[0]["lr"], on_step=True, prog_bar=False)
        # --- логирование (как раньше в training_step_end) ---
        # собираем метрики из обоих шагов
        log_info = {}
        log_info.update({k: v.mean() for k, v in out_g.get('log_info', {}).items()})
        log_info.update({k: v.mean() for k, v in out_d.get('log_info', {}).items()})
        if log_info:
            self.log_dict(log_info, on_step=True, on_epoch=False, prog_bar=False)

        # можно вернуть любой скаляр для прогресс-бара; используем loss_g
    ...
    def validation_step(self, batch, batch_idx, dataloader_idx):
        ...
        # ⚠️ Lightning 2.x не передаёт outputs в on_validation_epoch_end:
        # сохраняем их сами
        if not hasattr(self, "_val_outputs"):
            self._val_outputs = []
        # запишем индекс даталоадера, чтобы восстановить группы при агрегации
        out["_dataloader_idx"] = dataloader_idx
        self._val_outputs.append(out)
        return out

    # NEW API: инициализируем буфер перед валидацией
    def on_validation_epoch_start(self):
        self._val_outputs = []

    # NEW API: итоговая агрегация
    def on_validation_epoch_end(self):
        # соберём всё, что накопили в validation_step
        outputs = getattr(self, "_val_outputs", [])
        if not outputs:
            return
    ...
    def get_ddp_rank(self):
        """
        PL>=2.0: нет num_processes. Берём world_size (или num_nodes*num_devices) и возвращаем global_rank,
        иначе None для сингл-процесса.
        """
        # попробуем самый надёжный источник
        ...
        return self.global_rank if (world_size and world_size > 1) else None
