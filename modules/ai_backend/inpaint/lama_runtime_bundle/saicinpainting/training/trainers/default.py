"""
FILE HEADER (saicinpainting/training/trainers/default.py)
- Назначение: дефолтный training/inference модуль LaMa поверх BaseInpaintingTrainingModule.
- Для backend-инференса используется только `forward` в режиме `predict_only=True`.
- Тренировочные зависимости (`losses`, `OmegaConf`, `FakeFakesGenerator`) импортируются
  лениво, чтобы runtime_bundle не тащил лишние модули при обычном инференсе.
"""
