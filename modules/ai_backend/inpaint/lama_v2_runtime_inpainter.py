"""
File: modules/ai_backend/inpaint/lama_v2_runtime_inpainter.py
...
STANDALONE MODULE - MUST STAY SELF-CONTAINED.
This file lives inside `modules/ai_backend/inpaint/`, but it is never imported as
a member of that package: `inpaint/lama.py` loads it by file path through
`importlib.util.spec_from_file_location` under the module name
`mf_lama_inpainter_v2_runtime`, which gives it NO package context
(`__package__` is empty). A relative import here - `from ..runtime.paths import
program_root`, `from .lama import ...` - therefore raises
`ImportError: attempted relative import with no known parent package` at load
time. Do not "clean up" the absolute/`sys.path` bootstrap below into relative
imports; keep every dependency either standard-library, a third-party package,
or resolved through the `sys.path` entries prepared right here.
"""
...
# ----------------------------------------------------------------------------
# Что в файле:
# - `InpainterV2`: standalone runtime-обёртка LaMa для IPC-метода
...
#   - `modules/ai_backend/inpaint/lama_runtime_bundle` (локальный `saicinpainting`),
#   - текущая папка модуля,
#   - корень программы.
# ============================================================================

_MODULE_DIR = Path(__file__).resolve().parent
# `modules/ai_backend/inpaint/` -> parents[0] = ai_backend, parents[1] = modules,
# parents[2] = program root. Counted here rather than taken from
# `runtime.paths.program_root()` because this module is loaded standalone (see
# the file header) and cannot use a package-relative import.
_PROGRAM_DIR = _MODULE_DIR.parents[2]
# The vendored `saicinpainting` bundle is a sibling directory, so it moves with
# this file.
