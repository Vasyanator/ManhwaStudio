"""
FILE OVERVIEW: modules/ai_backend/ocr/manga.py
...
- `MangaOcrModel.from_pretrained` is called without a `dtype`, so transformers
  ...
  `runtime.rocm_mmap_transfer.move_module_to`, which is a strict no-op off ROCm;
  see that module for the amdkfd stall it works around.
- The ONNX weight root comes from `runtime.paths.program_root()`; this file must
  never count `parents[N]` itself.
"""
...
class MangaOcrService:
    ...
    def _models_root() -> Path:
        """Return the directory holding the MangaOCR ONNX export sub-directories.

        This is the sole, unconditional lookup for MangaOCR ONNX weights (there
        is no config-driven alternative), so the root comes from
        `runtime.paths.program_root()` rather than a local `parents[N]` count
        that would silently break whenever this file changes directory depth.
        """
        return program_root() / MODELS_DIR_NAME / ONNX_DIR_NAME / "MangaOCR"
