"""
File: modules/ai_backend/test_manga_ocr_service.py

Purpose:
Unit tests for the MangaOCR PyTorch runtime's device placement contract.

Main responsibilities:
- verify the CUDA/ROCm placement goes through `rocm_mmap_transfer.move_module_to`
...
Notes:
`manga_ocr_service` imports `jaconv` at module scope and the runtime imports
...
def _import_service_module():
    """Import `manga_ocr_service`, skipping the test when `jaconv` is absent."""
    try:
        from modules.ai_backend import manga_ocr_service
    except ImportError as exc:  # pragma: no cover - environment specific
        raise unittest.SkipTest(f"manga_ocr_service is not importable: {exc}") from exc
    return manga_ocr_service
