...
- `CANVAS_RIBBON_TAB` (`mod.rs`) is the ONE definition of the controls tab's identity, and the
  ...
  (`declare_ribbon_tab`), so the three call sites cannot drift. It owns no DEFAULT ARRANGEMENT:
  every canvas program tab declares tabs of its own, and a default layout is the dictionary its
  stored tab keys are resolved against, so each builds its own
  (`translation_default_dock_layout`, `cleaning_default_dock_layout`, `typing_default_dock_layout`)
  and all three give the ribbon panel the same id and left-edge anchor. A tab that wants to
...
- To change what the «Лента» tab SHOWS, edit `scene.rs::draw_ribbon_tab_body`; to change how big it
  ...
  Where the panel holding it sits by default is a per-program-tab decision, and all three tabs have
