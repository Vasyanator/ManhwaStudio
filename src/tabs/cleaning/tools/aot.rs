/*
FILE HEADER (cleaning/tools/aot.rs)
- Назначение: инструмент "AI удаление (AOT)" для вкладки cleaning на базе
  `RegionMaskInpaintToolBase`, с вызовом Python backend endpoint `/inpaint/aot`.
- Ключевые сущности:
  - `AotInpaintTool`: wiring инструмента (выделение, UI параметров, routing ввода).
  - `AotParams`: параметры AOT для backend (`inpaint_size`).
  - `run_aot(...)`: конвертация region image + mask в PNG/base64, HTTP POST в backend и
    разбор результата `image_png_base64`.
- Поведение:
  - Shift+ЛКМ по canvas открывает region mask editor (через `RegionMaskInpaintToolBase`).
  - В окне рисуется маска; кнопка `Обработать` запускает AOT через backend.
...
const AOT_ENDPOINT: &str = "/inpaint/aot";
const AOT_UNLOAD_ENDPOINT: &str = "/inpaint/aot/unload";
const AOT_CONNECT_TIMEOUT: Duration = Duration::from_millis(1500);
const AOT_READ_TIMEOUT: Duration = Duration::from_secs(300);
const AOT_WRITE_TIMEOUT: Duration = Duration::from_secs(20);
...
impl Default for AotParams {
    fn default() -> Self {
        Self { inpaint_size: 2048 }
    }
}

pub struct AotInpaintTool {
    inpaint_base: RegionMaskInpaintToolBase,
    params: AotParams,
    unload_status: Option<String>,
}

impl Default for AotInpaintTool {
    fn default() -> Self {
        Self {
            inpaint_base: RegionMaskInpaintToolBase::new("aot_inpaint", Some(8)),
            params: AotParams::default(),
            unload_status: None,
        }
    }
}
...
impl AotInpaintTool {
    fn run_aot(
        ...
        ai_models::ensure_aot(&config::models_dir())?;
        ...
        let response = post_json(AOT_ENDPOINT, &payload)?;
...
impl CleaningTool for AotInpaintTool {
    ...
    fn draw_ui(&mut self, ui: &mut egui::Ui) {
        ...
        ui.small("Обработка через Python AI backend: /inpaint/aot.");
    }
    ...
    fn draw_overlay_ui(
        ...
        inpaint_base.draw_overlay_ui_custom(
            ...
            {
                let params = *params;
                move |image, mask| Self::run_aot(image, mask, params)
            },
            move |ui| {
                ui.separator();
                ui.collapsing("Параметры AOT", |ui| {
                    ui.add(
                        WheelSlider::new(&mut params.inpaint_size, 256..=4096).text("inpaint_size"),
                    );
                });
                if ui.small_button("Выгрузить AOT из backend").clicked() {
                    *unload_status = match post_json(AOT_UNLOAD_ENDPOINT, "{}") {
                        Ok(_) => Some("Запрошена выгрузка AOT из памяти backend.".to_string()),
...
fn post_json(path: &str, payload: &str) -> Result<Value, String> {
    ...
    let mut stream = TcpStream::connect_timeout(&socket_addr, AOT_CONNECT_TIMEOUT)
        .map_err(|err| format!("Не удалось подключиться к AI backend: {err}"))?;
    stream
        .set_read_timeout(Some(AOT_READ_TIMEOUT))
        .map_err(|err| format!("Не удалось выставить read timeout AI backend: {err}"))?;
    stream
        .set_write_timeout(Some(AOT_WRITE_TIMEOUT))
