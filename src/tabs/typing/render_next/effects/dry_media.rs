    ...
    for y in 0..out_height_usize {
        for x in 0..out_width_usize {
            ...
            if src_a > 0 {
                ...
                out[rgba_idx + 1] = ((base_g as f32) * brightness).round().clamp(0.0, 255.0) as u8;
                out[rgba_idx + 2] = ((base_b as f32) * brightness).round().clamp(0.0, 255.0) as u8;
            ...
            let dust_band = 1.0 - smoothstep01(outside_dist / dry_media.dust_radius_px.max(0.001));
...
mod tests {
    ...
    use super::apply_dry_media_effect;
