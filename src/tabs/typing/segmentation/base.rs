...
impl Conservatism {
    ...
    #[must_use]
    pub(crate) fn all() -> [Conservatism; 4] {
        [
            Conservatism::Safe,
...
pub(crate) struct Joint {
    ...
    pub(crate) same_line: Cow<'static, str>,
    /// Что дописывается в хвост головной строки при переносе здесь (напр. `"-"` / `""`).
    pub(crate) wrap_suffix: Cow<'static, str>,
...
impl Joint {
    ...
    #[must_use]
    pub(crate) fn space(spaces: String) -> Self {
        Self {
    ...
    #[must_use]
    pub(crate) fn soft_hyphen(break_cost: u32) -> Self {
        Self {
    ...
    #[must_use]
    pub(crate) fn with_conservatism(mut self, conservatism: Conservatism) -> Self {
...
pub(crate) struct SegmentOptions {
    ...
    pub(crate) allow_hard_hyphen_breaks: bool,
...
#[must_use]
pub(crate) fn count_layout_units(text: &str, hanging_punctuation: bool) -> usize {
    text.chars()
        .filter(|&ch| {
            ch != SOFT_HYPHEN
                && ch != '\n'
...
#[must_use]
pub(crate) fn build_line_text_and_units(blocks: &[Block], wraps_here: bool) -> (String, usize) {
