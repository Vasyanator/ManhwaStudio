"""
Lightweight base utilities for the translation tab.

This replaces the old BallonsTranslator shim with a self-contained
implementation that exposes the tiny subset of functionality the local
textdetector modules need.
"""
    ...
    return deepcopy(
        {
            "type": "selector",
            "options": [
                opt for opt in AVAILABLE_DEVICES if all(device not in opt for device in not_supported)
            ],
...
def standardize_module_params(params):
    ...
    for k, v in params.items():
        if not isinstance(v, dict) and k not in {"description"}:
            v = {"value": v}
...
class BaseModule:
    ...
    def low_vram_mode(self) -> bool:
        if self.params is not None and "low vram mode" in self.params:
...
# Optional helpers retained for compatibility --------------------------------
